...
public class ElementQueryBuilder {
	...
	public ElementQueryBuilder(ContextIdentifier context, ORConfig mappingConfig,
	...
	private String buildWhere(Restriction restriction) {
		...
		if (restriction instanceof OrRestriction) {
			...
			if (er instanceof ClassIn) {
				...
				return alias(er.getElement()) + ".ClassId IN (SELECT ClassId FROM ClassIdLookup WHERE ClassName IN ("
				...
				if (pr instanceof FieldEq) {
					...
					return alias(pr.getElement()) + "." + col.getColumnName() + " = " + namer.param(val);
				} else if (pr instanceof Fieldike) {
					Fieldike propertyLike = (Fieldike) pr;
					return alias(pr.getElement()) + "." + col.getColumnName() + " like "
					...
					return alias(pr.getElement()) + "." + col.getColumnName() + " IN ( "
							+ StringUtils.join(paramNames, ",") + " ) ";
				} else {
	...
	/**
	 * Returns the SQL table alias for the table corresponding to this
	 * {@link ElementRef}, and also ensures that the table is registered in the
	 * FROM list, and that any rows from that table are within the context
	 * structure.
	 * It filters all REMOVED status
	 * It filters all REMOVED status
	 */
	public String alias(ElementRef element) {
		...
		// Filtering rows based on the specified context (change effective context or regular BC context)

		query.addWhere(alias + "." + mapping.getIdColumn() + " IN ("
				+ queryBuilder.elementVersionsInClause(context, namer) + ")");
		
		// we want to eliminate everything in REMOVED status
		query.addWhere(alias +  "." +"status!='REMOVED'");
