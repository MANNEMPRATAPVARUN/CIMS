...
/*
        function changeRequestCategory(){
...
*/
	...
	function download(){
		...
		var generationUrl = '<c:url value="/reports/reviewGroup/generateOutboundQuestionsReport.htm"/>';
    ...
    function popupReviewGroupOutboundQuestionsReport() {

	var validationUrl = '<c:url value="/reports/reviewGroup/validate.htm"/>';
	var generationUrl = '<c:url value="/reports/reviewGroup/generateOutboundQuestionsReportHtml.htm"/>';
	var checkUrl = '<c:url value="/reports/reviewGroup/checkDownloadProgress.htm"/>';
	    ...
	    success: function(data){
	    	if("FAILED"==data.status){
	    		...
	    		var url1 = generationUrl+"?token="+token+"&"+$('#viewForm').serialize();
                        var newwindow = window.open(url1, "Review Group Outbound Questions Report", "width=1380,height=750,resizable=no,scrollbars=yes, menubar=yes ");
                        newwindow.document.title = 'Review Group Outbound Questions Report';
	                if (window.focus)  {
	                    newwindow.focus();
	                }
...
<h4 class="contentTitle">Reports > Review Group Outbound Questions Report</h4> 

<div class="content">

<fieldset id="reportCriteria">
   <legend>Review Group Outbound Questions Report</legend>
	...
	<form:hidden path="languageDesc" id="languageDesc"/>
	 ...
	 <fieldset>	
	 	...
	 	<tr>
			...
			<td width="15%"><label for="requestCategory"><span class="required">*</span>Request Category:</label></td>
			<td width="15%">	
			<form:select  path="requestCategory" onchange="refreshForm();" id="requestCategory">
			<%--<form:select  path="requestCategory" onchange="changeRequestCategory();" id="requestCategory">--%>
	 	...
	 	<tr>
	 		<td>
		    <label for="reviewGroup"><span class="required">*</span>Review Group:</label>	
		    </td>
		    <td colspan="4">	
		...
		<tr>
			<td>
			<label for="patternTopic">Pattern Topic:</label>
			</td>
			<td colspan="4">
		...
		<tr class="tabular">
			<td colspan="4">
				<table>
					<tr><td><label>Code Range:</label></td><td><label>From:</label></td><td><form:input  path="codeFrom" size="10" maxLength="7"/></td><td><label>To:</label></td><td><form:input  path="codeTo" size="10" maxLength="7"/></td></tr>
				</table>
			</td>
			<td>&nbsp;</td>
		</tr>
		...
		<tr class="index">
			<td>
			<label for="indexBook">Index book:</label>
			</td>
			<td colspan="4">
		...
		<tr class="index">
			<td>
			<label for="leadTerm">Lead Index Term:</label>
			</td>
			<td colspan="4">
		...
		<input class="button" type="button" value="<fmt:message key='cims.reports.reviewgroup.html.submitButton'/>" 
		       onclick="javascript:popupReviewGroupOutboundQuestionsReport();"/>
