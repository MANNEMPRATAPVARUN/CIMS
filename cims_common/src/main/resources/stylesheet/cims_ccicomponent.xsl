...
<xsl:variable name="cciComponentId">
	<xsl:value-of select="normalize-space(cciComponent/@id)" />
</xsl:variable>

<xsl:variable name="language">
	<xsl:value-of select="normalize-space(cciComponent/@language)" />
</xsl:variable>

<xsl:variable name="clinical_classification_code">
	<xsl:value-of select="normalize-space(cciComponent/@classification)" />
</xsl:variable>
...
<xsl:template match="cciComponent">
	<xsl:apply-templates/>
</xsl:template> 
...
<xsl:template match="block">
  <tr>
		<td class='chp' colspan="3" >
		   <a><xsl:attribute name="name"><xsl:value-of select="$cciComponentId"/></xsl:attribute>
...
<xsl:template match="para/xref">
  ...
  <xsl:template match="td/ulist | ulist">
    ...
    -->
     <xsl:if test="label">
       <span class="includeSupp"><xsl:apply-templates select="label"/></span>
     </xsl:if>
       ...
       <table>
    		<xsl:if test="not(@tabstyle='header')">  
    		   <xsl:attribute name="class">
    		   	<xsl:text>cciComponentTable</xsl:text> 
    		   </xsl:attribute> 			
    		</xsl:if>		    		
