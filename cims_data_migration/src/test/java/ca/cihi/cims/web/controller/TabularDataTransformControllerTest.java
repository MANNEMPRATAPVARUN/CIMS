 ...
 * @author wxing
 */
public class TabularDataTransformControllerTest {
	private static final Log LOGGER = LogFactory.getLog(TabularDataTransformControllerTest.class);

	private TabularDataTransformController controller;
	...
	public void setUp() throws Exception {
		...
		controller = new TabularDataTransformController();
	...
	public void testSetupForm() {
		...
		assertSame(mav.getViewName(), "/migration/tabularDataTransformation");
	}
	...
	public void testTransformData() {
		...
		// Stub
		when(baseTransformService.checkRunStatus(FISCAL_YEAR_2009, Constants.CLASSIFICATION_ICD10CA)).thenReturn(false);
		// RunStatus is false
		ModelAndView mav = controller.transformData(model, FISCAL_YEAR_2009, Constants.CLASSIFICATION_ICD10CA);
		assertSame(mav.getViewName(), "/migration/tabularDataTransformation");

		// No tabular data
		when(baseTransformService.checkRunStatus(FISCAL_YEAR_2010, Constants.CLASSIFICATION_ICD10CA)).thenReturn(true);
		when(
		...
		when(transformService.getAllConcepts(Constants.CLASSIFICATION_ICD10CA, null)).thenReturn(null);
	...
	public void testViewTransformationReport() {
		...
		assertSame(mav.getViewName(), "redirect:/tabularTransformation.htm");
	}
